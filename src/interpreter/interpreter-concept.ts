...
class Substract implements IAbstractExpression {
  // Non-Terminal Expression.
  left: IAbstractExpression;
  right: IAbstractExpression;
...
// Manually Creating an Abstract Syntax Tree from the tokes
...
AST.push(new Substract(AST[0], new Numeral(TOKENS[4]))); // ^ -  3
AST.push(new Add(AST[1], new Numeral(TOKENS[6]))); //^ + 7
AST.push(new Substract(AST[2], new Numeral(TOKENS[8]))); // ^ -  2
